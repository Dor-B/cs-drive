    ...
    >
      <template v-slot:body.prepend v-if="nonEmptyHeaders.length > 1">
        <tr class="grey lighten-3">
          ...
          <td v-for="(header, i) in nonEmptyHeaders" :key="header.text">
      ...
      <template v-slot:item.fileName="{ item }">
        ...
        <a
          ...
          :href="viewUrlFromId(item.driveId)"
          :title="item.fileName"
        >
...
export default {
    ...
    filteredItems() {
      const filterMatchesItem = ([filterName, filterList], item) => {
        ...
        if (this.isCommaSeparatedField(filterName)) {
          return filterList.every(filter =>
            this.commaSeparatedToList(item[filterName]).includes(filter)
          );
        }
      ...
      if (!this.isMobile) {
        firstFiltered = firstFiltered.filter(item => {
          return Object.entries(this.filters).every(filterPair =>
            filterMatchesItem(filterPair, item)
          );
        });
      }
      if (this.search == "") return firstFiltered;
      return firstFiltered.filter(item =>
        isListMatchingQuery(this.search, this.headerDataList(item))
      );
    },
    filterAble() {
      return this.headers
        .filter(header => header["filterType"] == "multiple")
  ...
  methods: {
    ...
    columnValueList(field) {
      ...
      if (REVERSE_SORTED_FILTERS.includes(field)) sorted = sorted.reverse();
      return sorted;
    },
    viewUrlFromId(id) {
    ...
    fileIcon(mimeType) {
      if (mimeType && mimeType.startsWith("image")) return "mdi-image";
      const icons = {
      ...
      if (mimeType in icons) return icons[mimeType];
      return "mdi-file";
    },
    shortenFileName(filename) {
      const dots = "...";
      if (filename.length <= MAX_FILENAME_CHARS) return filename;
