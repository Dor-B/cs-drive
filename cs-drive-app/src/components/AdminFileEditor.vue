  ...
  <v-expansion-panels accordion>
    <v-expansion-panel>
      <v-expansion-panel-header
        >מחיקה ועריכת קבצים קיימים</v-expansion-panel-header
      >
...
export default {
  ...
  data() {
    return {
      ...
      driveLinkRules: [
        ...
        v =>
          [...v.matchAll(driveLinkReg)].map(x => x.groups).length > 0 ||
          "הקישור איננו תקין"
      ],
  ...
  computed: {
    driveIdToSearch() {
      const idMatches = [...this.driveLink.matchAll(driveLinkReg)].map(
        x => x.groups
      );
      if (idMatches.length > 0) return idMatches[0].id;
      return null;
    }
  },
  asyncComputed: {
    fileInfo: {
      get() {
        if (!this.currentCourseId || !this.driveIdToSearch) return null;
        const that = this;
        return db
          .ref(`courses/${that.currentCourseId}/directories`)
          .once("value")
          .then(snap => snap.val())
          .then(dirObj => {
    ...
    fileMetadata: {
      get() {
        ...
        return db
          .ref(
            `courses/${this.currentCourseId}/directories/${directory}/${key}`
          )
          .once("value")
          .then(snap => snap.val())
          .then(metadata => ({
            courseId: that.currentCourseId,
            driveId: that.driveIdToSearch,
            directory,
            ...metadata
          }));
      },
