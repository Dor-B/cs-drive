  ...
  <div>
    ...
    <v-card v-if="showComponent || !hideOnFinish">
      ...
      <v-form v-model="isFormValid">
        <v-card-text>
          <v-text-field v-model="fileData.fileName" label="שם הקובץ" light>
          </v-text-field>
          <FileMetadataEditor
            v-model="fileData"
      ...
      <v-card-actions>
        <v-btn @click="_accept" color="primary" text outlined>
          <v-icon left>mdi-check</v-icon>
          אשר
        </v-btn>

        <v-btn @click="_remove" color="primary" text>
          מחק
        </v-btn>
      </v-card-actions>
    </v-card>
    <v-snackbar v-model="showUndo">
      {{ snackbarText }}
      <v-btn color="primary" text @click="undo">
        בטל
      </v-btn>
    </v-snackbar>
  </div>
  ...
  computed: {
    formDirHelper() {
      return new FormDirHelper(
        this.fileData.courseId,
        this.fileData.directory,
        this.fileData,
