      ...
      <v-menu bottom left>
        ...
        <v-list dense>
          <v-list-item id="feedback-menu-item" @click="openFeedback()">
            <v-list-item-icon
              ><v-icon>mdi-comment-alert</v-icon></v-list-item-icon
            >
    ...
    <v-content>
      <v-container fill-height fluid class="grey lighten-4">
        ...
        <v-row
          justify="center"
          align="center"
          :class="{ 'desktop-table-row': !isMobile }"
        >
          <v-expand-transition>
            <v-card
              ...
              v-if="courseHasItems"
            >
              <v-tabs v-model="tab" hide-slider show-arrows center-active>
            ...
            <p v-else>
              אין עדיין קבצים בקורס זה, בואו לתרום ולעלות קבצים דרך הכפתור משמאל
              למעלה!
            </p>
          </v-expand-transition>
        </v-row>
...
export default {
  ...
  computed: {
    ...
    coursesItemsByLastSeen: function() {
      const that = this;
      let lastSeenItems = [...this.lastCourses]
        .sort()
        .map(id => that.coursesItems.filter(i => i.value == id)[0]);
      let otherItems = this.coursesItems.filter(
        item => !that.lastCourses.includes(item.value)
      );
      return [...lastSeenItems, ...otherItems];
    },
    tableTitle: function() {
      if (
        isEmpty(this.currentCourseInfo) ||
        isEmpty(this.namesMap) ||
        this.isMobile
      ) {
        return "";
      }
      return (
        this.currentCourseInfo.name +
        " - " +
        this.currentCourseId +
        " \\ " +
        this.namesMap[this.currentCourseDir]
      );
    },
    ...
    tabs: function() {
      if (
        isEmpty(this.currentCourseInfo) ||
        !("directories" in this.currentCourseInfo)
      )
        return [];
      const that = this;
      // filter only tabs with 1 or more files
      return this.currentCourseInfo.directories
        .filter(
          name => name in that.dirsNumChildren && that.dirsNumChildren[name] > 0
        )
        .map(name => {
          return { name: name, text: this.namesMap[name] };
        });
    },
  ...
  watch: {
    currentCourseId: function(newCourse) {
      if (newCourse != null) {
        this.lastCourses = [
          newCourse,
          ...this.lastCourses.filter(c => c != newCourse)
        ].slice(0, NUM_LAST_COURSES);
      }
    }
  },
  asyncComputed: {
      ...
      get() {
        return fbValue(
          "courses/" +
            this.currentCourseId +
            "/directories/" +
            this.currentCourseDir
        ).then(obj =>
          Object.entries(obj).map(([key, item]) => ({ ...item, key }))
        );
      },
      default: []
    },
    headers: {
      get() {
        const getWidth = headerValue =>
          COL_WIDTHS[headerValue] ? { width: COL_WIDTHS[headerValue] } : {};
        return fbValue("headers/" + this.currentCourseDir).then(hs =>
    ...
    dirsNumChildren: {
      get() {
        return db
          .ref("courses/" + this.currentCourseId + "/directories")
          .once("value")
          .then(snap => {
            let activeDirs = [];
            snap.forEach(childSnap => {
              activeDirs.push([childSnap.key, childSnap.numChildren()]);
            });
