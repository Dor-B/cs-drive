...
export function fbPathHasChild(path, childPath) {
  return db
    .ref(path)
    .once("value")
    .then(snap => snap.child(childPath).exists());
}

export function getFbCourseDirectories(courseId) {
  if (!isNonEmptyStr(courseId)) return Promise.resolve([]);
...
export function sortWithNumbersCmp(a, b) {
  ...
  if (bothNums) return defaultSortCmp(aNum, bNum);
...
export function isListMatchingQuery(queryText, list) {
  ...
  let matches = queryList.filter(q => list.some(field => field.includes(q)))
    .length;
...
export function getAppsScriptIframeUrl(baseUrl, paramsObj, metaParamsObj) {
  ...
  for (let [key, data] of Object.entries(paramsObj)) {
    if (data) url.searchParams.append(key, data);
  }
...
export class LocalStorage {
  ...
  static getComputedField(localStorageInstanceName) {
    return {
      get: function() {
        return this[localStorageInstanceName].val;
      },
      set: function(val) {
        this[localStorageInstanceName].val = val;
      }
    };
  }
}
