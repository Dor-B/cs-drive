...
const startOfSpringNextYear = new Date(
  someYear + 1,
  monthNames.indexOf("March"),
  18
);
...
export class FormDirHelper {
  ...
  getDefaultString(inputHeader) {
    ...
    switch (inputHeader) {
      case "year": {
        ...
        if (
          ["November", "December"].includes(monthName) ||
          (monthName == "October" && dayOfMonth > 20)
        ) {
      ...
      case "semester": {
        ...
        if (now >= startOfWinter && now < startOfSpringNextYear) return "חורף";
        if (now >= startOfSpring && now < startOfSummer) return "אביב";
        return "קיץ";
      }
    ...
    if (inputHeader == "comments") {
      return [
        comment =>
          ...
          `הכנס עד ${MAX_COMMENTS_LENGTH} תווים`
      ];
    }
    // default is no rule
    return [];
  ...
  getInputType(inputHeader) {
    if (inputHeader == "number") return "number";
    return "other";
  }
}
